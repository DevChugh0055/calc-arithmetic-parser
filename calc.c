...
Token get_next_token(Parser *parser) {
    Token token;
    token.start_pos = parser->pos + 1; // Convert to 1-based
    ...
    if (isdigit(current) || current == '.') {
        ...
        if (end == parser->input + parser->pos) {
            // No number parsed - this shouldn't happen with our check
...
void process_file(const char *input_path, const char *output_dir, 
    ...
    while (fgets(input_buffer + total_chars, MAX_INPUT_SIZE - total_chars, input_file)) {
        int line_len = strlen(input_buffer + total_chars);
        
        // Skip comment lines
        if (!has_content && is_comment_line(input_buffer + total_chars)) {
            continue;
        }
        ...
        if (total_chars >= MAX_INPUT_SIZE - 1) {
            break;
        }
    }
    fclose(input_file);
    ...
    input_buffer[total_chars] = '\0';
    
    // Parse and evaluate
    Parser parser;
    init_parser(&parser, input_buffer, total_chars);
